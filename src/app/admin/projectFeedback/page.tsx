...
export default function AllprojectFeedbackPage() {
  ...
  const fetchprojectFeedback = async () => {
    try {
      ...
      setProjectFeedback(res.data.projectFeedback);
    } catch (err) {
      console.error("Error fetching projectFeedback:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const deleteProjectFeedback = async (id: number) => {
    if (!confirm("Are you sure you want to delete this Project Feedback?"))
      return;

    try {
      await axios.delete(`/api/v1/admin/projectFeedback/${id}`);
      toast.success("ProjectFeedback deleted");
      setProjectFeedback((prev) =>
        prev.filter((ProjectFeedback) => ProjectFeedback.id !== id)
      );
    } catch (err) {
      console.error("Error deleting ProjectFeedback:", err);
      toast.error("Failed to delete ProjectFeedback");
    }
  };
  ...
  return (
    <div className="p-6">
      <h1 className="text-2xl font-bold mb-4">All Project Feedback</h1>
      {loading ? (
        <p>Loading...</p>
      ) : projectFeedback.length === 0 ? (
        <p>No projectFeedback found.</p>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {projectFeedback.map((ProjectFeedback) => (
            <div
            ...
            >
              ...
              <img
                src={ProjectFeedback.image}
                alt={ProjectFeedback.name}
                className="w-full h-40 object-cover mb-2"
              />
              <div className="w-full bg-red-50 flex flex-row">
                <div className="flex items-center justify-center px-5">
                  <img
                    src={ProjectFeedback.projectImage}
                    alt={ProjectFeedback.name}
                ...
                <div className="w-1/2 pl-8">
                  ...
                  <p>{ProjectFeedback.description}</p>
                  <p>{ProjectFeedback.linkedInUrl}</p>
                </div>
              </div>
              <button
                onClick={() => deleteProjectFeedback(ProjectFeedback.id)}
